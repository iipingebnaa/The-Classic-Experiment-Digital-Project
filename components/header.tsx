...
import { Menu, X, Home, Phone, ShoppingCart } from "lucide-react";
...
export default function Header() {
  ...
  const currentCustomer = store.getState().auth.customer;
  if (!currentCustomer) {
    dispatch(setIntent({ action: INTENTS.GO_TO_ORDER }));
    router.push("/login");
    setMenuOpen(false);
    return;
  }
  ...
  const currentCustomer = store.getState().auth.customer;
  if (!currentCustomer) {
    dispatch(setIntent({ action: INTENTS.GO_TO_MY_ORDERS }));
    router.push("/login");
    setMenuOpen(false);
    return;
  }
    ...
    <header className="fixed top-0 left-0 right-0 bg-white/40 backdrop-blur-sm z-50 px-1 md:px-12 flex items-center justify-between shadow-sm h-18 md:h-20">
      ...
      <nav className="hidden md:flex gap-6 items-center pr-4">
        ...
        {customer ? (
          ...
          <button
            onClick={handleOrderClick} // uses safeNavigate
            className="bg-[#003262] text-white px-4 py-2 rounded-full font-medium shadow-md hover:brightness-110 transition text-md"
          >
            Start Order
          </button>
